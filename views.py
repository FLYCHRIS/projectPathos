...
def addEquipmentPage(request):
    ...
    # action carried out when client submits form
        ...
        #while request.method == 'POST' and form2.is_valid() and not form1.is_valid():
            #docFiles = files
                ...
                #for f in newDocFiles:
                    #with open(
                            #'/Users/TryingNerd24/Documents/Personal Projects/Coding/Webpage Development/projectPathos/projectPathosApp/projectPathosApp/media/equipDocs/' + str(f), 'wb+') as destination:
        ...
        if form1.is_valid() and form2.is_valid() and len(addEquipmentClass.objects.filter(tagID=tagID)) <= int(0):
            ...
            # adds document name to fileList and stores the document to the MEDIA location where the app stores files on system.
            for f in files:
                fileList.append(str(p))
                with open('/Users/TryingNerd24/Documents/Personal Projects/Coding/Webpage Development/projectPathos/projectPathosApp/projectPathosApp/media/equipDocs/' + str(f), 'wb+') as destination:
                    for chunk in f.chunks():
                        destination.write(chunk)
